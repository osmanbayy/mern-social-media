...
const FollowingModal = ({ user, authUser, isOpen, onClose }) => {
  ...
  const handleUserClick = (username) => {
    document.getElementById(`following_modal${user._id}`).close();
  ...
  const isFollowingUser = (itemId) => {
    return authUser?.following?.includes(itemId);
  };
  ...
  return (
    ...
    >
      <div className="modal-box p-6 max-w-screen-sm flex flex-col gap-4 bg-base-100 shadow-2xl">
        <h3 className="font-bold text-xl mb-1">Takip Edilenler</h3>
        {isFollowingLoading && <LoadingSpinner size="lg" />}
        {!isFollowingLoading && followings?.length === 0 && (
          <p className="text-center text-base-content/60 py-8">
            Takip edilen hesap yok.
          </p>
        )}
        {followings &&
          !isFollowingLoading &&
